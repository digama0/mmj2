...
$( Comments welcome; email to:  solovay at math dot berkeley dot edu $)
...
$( 24-Jun-2006 (N. Megill) - Made label and math symbol names spaces
       ...
       respectively. $)
$( 04-May-2008 (M. O'Cat) - Cloned to create peanoInfix.mm $)
$( 17-May-2008 (M. O'Cat) - Eliminated parentheses on wff_atom and other
                            usages of binpred after N. Megill pointed out
                            that binary predicates (e.g. "=" and "<") 
                            create wffs and thus no ambiguity results. $)
...
$)
wff_logbinop $a wff ( psi logbinop phi ) $.
...
$)
ax-1 $a |- ( phi implies ( psi implies phi ) ) $.
...
ax-2 $a |- implies
              ...
              implies
                 ...
                 implies phi chi  $.
$)
ax-2 $a |- ( ( phi implies ( psi implies chi ) )
             implies
             ( ( phi implies psi ) 
               implies 
               ( phi implies chi ) 
...
ax-3 $a |- implies
              ...
              implies psi phi $.
$)
ax-3 $a |- ( ( not phi implies not psi )
             implies 
             ( psi implies phi ) 
           ) $.
...
$)
eq-binop $a |- ( ( s0 = s1 and t0 = t1 )
               implies 
                 ( s0 binop t0 ) = ( s1 binop t1 )
               ) $.
...
${ $d phi x $.
   ...
   alpha_hyp2 $e |- ( phi implies chi ) $.
   alpha_2    $a |- ( phi implies forall x chi ) $.
$}
...
${ $d phi y $.
   $d psi x $.
   $d x y $.
   alpha_3 $a |- ( forall x forall y ( x = y implies ( phi iff psi ) )
                 implies 
                   ( quant x phi iff quant y psi )
                 ) $.
$}
...
$d x t $.
 ...
 all_elim3_hyp1 $e |- ( x = t
                      implies 
                        ( phi iff chi )
                      ) $.
all_elim3 $a |- ( forall x ( x = t implies phi )
                implies 
                  chi 
                ) $. $}
...
$d x y $.
induction $a
|- ( ( forall y ( y = 0 implies phi )
     and 
       forall x ( forall y ( y = x implies phi )
                implies 
                  forall y ( y = S x implies phi ) 
                )
     ) 
   implies 
     forall x forall y ( y = x implies phi )
...
$( NEW -- so that mmj2 has at least one theorem to work with $)
${
  dummy.1 $e |- phi $. 
  dummy   $p |- phi
$=  wff_phi wff_phi dummy.1 wff_phi logbinopimplies wff_phi wff_phi 
    wff_logbinop dummy.1 wff_phi wff_phi ax-1 ax-mp ax-mp $. 
$}
