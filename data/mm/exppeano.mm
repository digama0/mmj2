...
$( Comments welcome; email to solovay@math.berkeley.edu $)

$( Version of 13-July-04 $)
$( 7-Oct-04 (N. Megill) - Minor fixes to conform to strict Metamath spec $)
$( 11-Oct-04 (N. Megill) - "$a implies" --> "$a |- implies" at line 224 $)
...
$( The next set of axioms are devoted to the proof of Lemma 1 of the
first pass. Roughly Lemma 1 says that if phi is a well formed formula of PA
that does not contain the variable x free then metamath can prove
    phi -> A. x phi $)

${
$d x phi $.
no_x_in_phi $a |- implies phi forall x phi $.
$}

no_x_if_bound $a |- implies quant x phi forall x quant x phi $.

${
ass_no_x $e |- implies phi forall x phi $.
no_x_through_not $a |- implies not phi forall x not phi $.
no_x_through_quant $a |- implies quant y phi forall x quant y phi $.

${
ass2_no_x $e |- implies psi forall x psi $.
no_x_through_logbinop $a |- implies
                            logbinop phi psi
                            forall x logbinop phi psi $.
$}
$}

$( Using these axioms it is straightforward to prove the following
lemma:

Lemma 1.
Let PHI be a well-formed formula of PA. [So the only variables
appearing in PHI are of kind var.] Suppose that PHI does not contain
the variable x free. Then Metamath [from the axioms given so far] can
prove the expression
  |- implies PHI forall x PHI
$)
...
following speccial case: Phi_2 is trim and no bound variable of Phi_2
...
$( I need a pedantic fact:
   ...
   The proof is by inductionon j - i and is straightforward. [It
...
The cases when Psi_1 is built up from smaller formulas using logical
...
$( Using this axiom we can deduce from (A) the following:
...
$( From this axiom, (B) and Propositions 2.3 and 2.4 we easily deduce:
...
from the disjointness assumption that all the distinctvariables
appearing in Phi_1 or Phi_2 are asserted to be distinct in an $d
assumption.  $)
...
of phi.}
   ...
   We say that t is substituable for x at the free occurrence of x in
   phi [or just "substituable" if we are being terse] if no variable
   ...
   **where** t is substituable for x in phi.

Using our result about the provability of the equivalence of
alpha-equivalent formulas, we can reduce matters to the following
special case [by replacing forall x phi by a suitable alpha
equivalent [and changing x, if necessary, to a different variable].

1) x does not appear in t;
